...
const ALLOWED: Array<"admin"> = ["admin"];
...
export default function KonfigurationPage() {
  ...
  return (
    <main style={wrap}>
      <Header title="Konfiguration" role={role} canWrite={canWrite} />
      <Card>
        <p style={lead}>Platzhalter – Konfiguration folgt.</p>
        <p style={text}>Beispiel: Systemparameter, Rollenverwaltung, Whitelist, Logos, Farben…</p>
...
function Actions({ canWrite }: { canWrite: boolean }) {
  return (
    <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
      ...
      {!canWrite && <span style={{ color: "#64748b", marginLeft: 8 }}>Kein Zugriff (nur Admin)</span>}
    </div>
  );
}
